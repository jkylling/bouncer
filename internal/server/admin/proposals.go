...
//
// Tier wiring:
//   - Create / read-list / read-one / update / delete require any
//     authenticated caller. Subject scoping (only your own proposals
//     unless admin) is applied inside the handlers via the Caller
//     in ctx — see phase 5.
//   - Approve / Reject are admin-only: a reviewer decision lands a
//     policy in the runtime, which is the operator-controlled tier.
//   - The HTML shell stays open so the login JS can render.
func MountProposals(r chi.Router, svc *proposals.Service) {
	r.Get(ProposalsPath, RequireAuthenticated(listProposalsHandler(svc)))
	r.Post(ProposalsPath, RequireAuthenticated(createProposalHandler(svc)))
	r.Get(ProposalItemPath, RequireAuthenticated(getProposalHandler(svc)))
	r.Patch(ProposalItemPath, RequireAuthenticated(updateProposalHandler(svc)))
	r.Delete(ProposalItemPath, RequireAuthenticated(deleteProposalHandler(svc)))
	r.Post(ProposalApprovePath, RequireAdmin(approveProposalHandler(svc)))
	r.Post(ProposalRejectPath, RequireAdmin(rejectProposalHandler(svc)))
