...
// contract (ErrNotFound semantics, JSON payload round-trip, filter
// behaviour) is uniform across the two.
//
...
func NewMemoryStore() Store {
	...
	if err != nil {
		panic(fmt.Errorf("proposals: open :memory:: %w", err))
	}
	s, err := newSQLiteStore(b)
	if err != nil {
		_ = b.Close()
		panic(fmt.Errorf("proposals: migrate :memory:: %w", err))
	}
	return s
}
