...
// Delete removes the proposal at id. ErrNotFound mirrors MemoryStore
// — a missing id is reported rather than silently accepted, since
// the Service uses Delete for the explicit purge path and a
...
// returned row so callers see the same shape MemoryStore returns.
