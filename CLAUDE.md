...
- **`bouncer serve` defaults `--data-dir` to cwd** when cwd looks
  initialized (secret.hex + admin-password.hash present). Lets an
  operator drop into their data dir and run a bare `bouncer serve`.
  An explicit flag or `$BOUNCER_DATA_DIR` always wins; an empty /
  uninitialized cwd is never silently consumed.
